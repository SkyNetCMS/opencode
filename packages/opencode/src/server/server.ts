...
import { basePathMiddleware, composeMiddleware } from "./routes/instance/httpapi/middleware/base-path"
...
type ListenOptions = CorsOptions & {
  ...
  mdns?: boolean
  mdnsDomain?: string
  // SkyNetCMS: base path prefix for serving behind a reverse proxy. Propagated
  // to RuntimeFlags via OPENCODE_BASE_PATH so the per-listener ConfigProvider
  // (ConfigProvider.fromEnv) reads it.
  basePath?: string
}
type ListenerState = {
  scope: Scope.Scope
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  // SkyNetCMS: surface basePath through the environment so the per-listener
  // ConfigProvider.fromEnv() (installed in listenerLayer) makes it available to
  // RuntimeFlags, which the basePath middleware and UI rewriters consume.
  if (opts.basePath !== undefined) process.env["OPENCODE_BASE_PATH"] = opts.basePath
...
function listenerLayer(opts: ListenOptions, port: number) {
  return HttpRouter.serve(HttpApiApp.createRoutes(opts), {
    // SkyNetCMS: basePathMiddleware strips the prefix before routing.
    middleware: composeMiddleware(basePathMiddleware, disposeMiddleware),
    disableLogger: true,
