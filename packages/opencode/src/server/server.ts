...
export namespace Server {
  ...
  export async function listen(opts: {
    ...
    let target: Hono = built.app
    if (base) {
      const wrapper = new Hono()
      wrapper.route(base, built.app)
      // Also mount at root for reverse proxies that strip the basePath
      wrapper.route("/", built.app)
      target = wrapper
    }

    const start = (port: number) =>
      new Promise<ServerType>((resolve, reject) => {
        const server = createAdaptorServer({ fetch: target.fetch })
