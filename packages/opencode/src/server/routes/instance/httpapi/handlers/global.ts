...
export const globalHandlers = HttpApiBuilder.group(RootHttpApi, "global", (handlers) =>
  Effect.gen(function* () {
    ...
    const health = Effect.fn("GlobalHttpApi.health")(function* () {
      // SkyNetCMS: read basePath synchronously from env (set per-listener by
      // Server.listen) to avoid adding a RuntimeFlags dependency to this handler.
      return {
        healthy: true as const,
        version: InstallationVersion,
        basePath: normalizeBasePath(process.env["OPENCODE_BASE_PATH"]) || undefined,
      }
    })
